// Shadertoy渲染脚本（shadertoy-diagram）
// 在文档加载完成后初始化
(function() {
    'use strict';
    ...
    function init() {
        ...
        if (typeof jQuery === 'undefined') {
            console.warn('shadertoy-diagram: jQuery is not loaded. Skipping shadertoy rendering.');
            return;
        }
        ...
        console.log(`shadertoy-diagram: Found ${containers.length} shadertoy figure(s)`);
        ...
        containers.forEach((container, index) => {
            try {
                ...
                console.error(`shadertoy-diagram: Error processing container ${index}:`, error);
            }
        });
    }

    function processContainer(container, index) {
        // 1. 提取GLSL源码 — 支持多种DOM结构
        let glslSource = '';

        // 检查是否有 <code> 元素（来自Typst渲染或hljs染色）
        const codeEl = container.querySelector('code');
        if (codeEl) {
            glslSource = codeEl.textContent.trim();
        } else if (isPlainText(container)) {
            glslSource = container.textContent.trim();
        } else {
            // 有非文本子元素但无 <code>：降级使用 textContent（去除所有HTML标签）
            console.warn(`shadertoy-diagram: Container ${index} has non-text elements, falling back to textContent.`);
            glslSource = container.textContent.trim();
        }

        if (!glslSource) {
            console.warn(`shadertoy-diagram: Container ${index} is empty. Skipping.`);
            return;
        }

        // 2. 保存原始子内容（Typst 渲染出的带语法高亮的 <pre><code>），稍后原样挂回代码面板
        const $original = $(container).children();

        // 3. 清理容器内容（移除原始文本）
        $(container).empty();

        // 4. 创建可折叠的代码显示区域，并把原始内容重新挂载到代码面板
    ...
    function createCodeView($container, $original, glslSource, index) {
        // 创建代码容器
        ...
        // 重新挂载原始子内容：Typst 已为代码渲染语法高亮，直接复用即可，无需 highlight.js
        if ($original && $original.length > 0) {
            $codeContent.append($original);
        } else {
            // 兜底：容器内没有元素子节点时，用提取到的文本生成普通代码块
            const $pre = $('<pre>');
            const $code = $('<code>', {
                class: 'language-glsl',
                text: glslSource
    ...
    function setupWebGL(canvas, glslSource, index) {
        ...
        if (!gl) {
            console.error(`shadertoy-diagram: WebGL2 not supported for canvas ${index}`);
        ...
        if (!program) {
            console.error(`shadertoy-diagram: Failed to create shader program for canvas ${index}`);
            return null;
        }
...
void main() {
    ...
    mainImage(fragColor, fragCoord);
        ...
        if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
            console.error('shadertoy-diagram: Program link failed:', gl.getProgramInfoLog(program));
    ...
    function compileShader(gl, type, source) {
        ...
        if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
            console.error('shadertoy-diagram: Shader compile failed:', gl.getShaderInfoLog(shader));
