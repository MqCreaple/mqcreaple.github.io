// title: 工程代码中的低级错误
// summary: 好的代码，应该让编译器直接禁止掉不合法的操作。
// tags: computer-science, programming-language, software-engineering
// category: tech
...
例如在 C++ 中，你能在不查文档的情况下告诉我，以下语句中`b`仅复制了`a`的地址，还是`b`复制了`a`的值？
...
以及如下的 Java 语句中，`b`仅复制了`a`的地址，还是`b`复制了`a`的值？
...
2. 复制值
3. 复制地址
4. 复制地址，但是 Java 的`String`类每次更改都会把它变成一个新的对象，所以基本上等价于复制值
...
因为在 Java 里，二维数组实际上是一个“数组的数组”。在第一层数组里，每个元素都是一个指针，指向着第二个数组的起始位置。这里的`clone`方法只是复制了第一层数组，但是它所指向的第二层数组的地址仍然是相同的。因此，修改`b`的某个具体元素仍然会改变`a`对应位置的元素。
...
为了方便起见，我们称第一种可变为_可赋值_，第二种可变为_可写_。

不同的编程语言在“可变性”上也有不同的规则

- 有一些语言并不区分变量的可变性，比如 Java 语言本身没有约束任何变量的可变性，代码的编写者通过写 getter 和 setter 来限制变量是否可写。
- 另一些语言只区分两种可变性的一种，如 Kotlin 的`var`和`val`、Javascript 的`let`和`const`只区分可否更改，而 Rust 的 immutable 则同时约束“可赋值”和“可写”两条属性。
- 有一些语言则在语法层面上同时规范了两种可变性。例如 C 语言的指针，`const char*`和`char const*`表示指针指向的内存不可写，而`char* const`表示指针本身的值不可改。

== 可变性导致的错误

没有正确处理变量是否可变也是代码错误的一大根源。比如很多初学 Java 的人可能会犯下面这样的错误：
...
错误的原因是：使用 for-each 循环来遍历一个数组的时候，数组本身在循环的局部作用域里应当是不可变的。否则的话就会引起歧义：这些新加进来的数应不应该也一起被循环遍历一遍？还是说这个循环只应该遍历原来的数，不去遍历新加入的数？

由于 Java 语言本身没有对变量的可变性做出任何约束，这段代码在编译的时候是可以通过的，但是一旦运行起来就会抛出异常。而对于可变性有更强约束的 Rust，同样的代码就连编译也过不了：
...
有时候你可能会想让同一个对象在某些语境下是可变的，某些语境下是不可变的，例如之前的 for-each 循环的例子。这个时候，标记对象的成员变量和成员函数的可变性就显得很重要了。

在许多面向对象语言中，改变特定成员的权限（`public`, `private`, etc.）可以约束用户的行为，让用户无法做出特定的改变。比如如果我们想要在 Java 里写一个字符串类，这是没有加权限说明的版本：
...
而在其他语言中，例如 C++，可能会有更好的解决方案。C++ 里在函数定义的末尾加上`const`关键字可以将它声明成一个“只读成员函数”。C++ 的编译器保证了这个函数里只能读取成员变量但是不能更改成员变量。比如上面的字符串的例子：
...
- 创建线程：传入一段需要执行的代码（通常传入一个无参数的函数或者 lambda 表达式），操作系统会在当前线程之外创建一个新的线程来并行执行这段代码。
- 结束线程：结束当前线程
- 等待线程：当前的线程一直等待到另一个线程结束执行。

频繁的创建\/结束线程会有较大的时间和空间开销，因此，能用尽量少线程完成的任务就不要开太多非必要的线程。通常 CPU 会有一个*硬件并行数*，表示 CPU 在硬件层面上最多支持多少线程同时运行，一般来说一个程序的线程数量如果超过 CPU 的硬件并行数，再开更多的线程就不会提升程序运行速度了。

如果需要执行的任务数量远超过线程数量，并且这些任务之间没有较强的依赖关系的话，那么可以尝试使用*线程池（thread pool）*。线程池会预先开出来很多线程，并且将所有任务存在一个队列里并依次分配给每个线程，保证每个线程在每个时刻只会执行一个任务。
...
虽然不同的线程可能会跑在不同的 CPU 核心上，但是它们都共享同一块内存。如果两个线程都知道某个全局变量的地址，那么它们就可以去尝试读取和写入这个变量。这样就有可能在两个线程之间传递信息了。比如下面的 C++ 代码：
...
互斥锁还有一些变体，比如_读写锁（read-write lock）_，它的“上锁”状态有“读”和“写”两种模式，它允许多个线程同时将它锁到“读”模式，但在任何时刻只有至多一个线程能把它锁到“写”模式。这保证了多个线程可以同时读取某个地址，但只能有一个线程在写入这个地址，并且不会有一个线程在读的时候另一个线程在写。这在保证线程同步的时候也略微提升了一些性能。
...
1. 线程1，读取`global`地址
2. 线程1，计算`global + 1`
3. 线程2，读取`global`地址（此时线程1还没有写回`global`，所以现在在内存中`global`的值还没变）
4. 线程1，写回`global`地址
5. 线程2，计算`global + 1`
6. 线程2，写回`global`地址
...
我们只需要将之前的代码里面的`int`替换成 atomic 标准库里的`atomic<int>`即可：
...
其实这些问题每一个单拎出来，都是编程语言里非常基础的问题，每一个编程语言的学习者应当对它们都了然于胸了。但是，一旦一个项目变得大起来、复杂起来，想要在一大堆代码中找出一个这样的小错误具体在哪就变得越来越困难，每一个这样看似微不足道的 bug 可能都需要花费几个小时甚至几天的时间仅仅是将它们定位出来。
...
- 在 Java 中，任何变量都要么是原始类型（`int`, `char`, `float`, etc.），要么是一个对象。
  - 在类的外面只能访问一个对象的`public`成员变量和调用`public`成员函数
  - 对象在传参的时候永远使用引用传参
- 在 C++ 中，变量可能是原始类型，对象，任何类型的指针，任何类型的引用，任何类型的右值引用，任何类型的常值类型，任何类型的常值指针（`const T*`），任何类型的常值引用（`const T&`）等等
...
这也在另一方面说明了，我们的世界不可能只拥有一种编程语言，而是需要许多种不同的编程语言根据程序员的需求来填补不同的位置：在某些应用场景下就是需要简单易学的编程语言，在某些应用场景下就是需要尽量精确而不易出错的编程语言。而学习者在初学编程语言的时候通常也有一条确定的路径：先从简单易学的编程语言（如 python）开始，一步步向后学习更多更底层、更具体的规则，接触其他的编程语言。
