...
// summary: 记录参加 MIT battlecode 竞赛的经历。
...
#image("battlecode-rank.png", width: 70%)

最近几天由于打竞赛打到自闭了（我太菜了），想整点其他的事情做一做，在学校信息社的推荐下打算玩一下 MIT 办的 #link("https://battlecode.org/")[battlecode] 比赛。

battlecode 是一个竞技性比赛。在一张固定大小的地图上，两方各有很多不同功能的机器人，而最终的目的是操作机器人占领尽可能多的天空岛（sky island）。只不过玩家不能直接操控，而是要为场上的机器人写代码，让程序自动运行。
...
- 每个格子可能是如下几种之一：
  ...
  - 障碍物：机器人不可通过
- 机器人的可视范围非常有限，但是机器人可以知道自己的`x`、`y`坐标以及地图大小
- 机器人之间不会自动共享信息，但是可以通过写入\/读取_共享内存_来传递信息
  - 共享内存为64个16位整数，即一共有128byte
  - 所有机器人都可以从共享内存中读取信息，但是只有在总部（headquarter）和信号增幅器（amplifier）附近可以写入内存
- 机器人每回合内不能做过多计算。battlecode 中直接使用 java 的 bytecode 数量来衡量计算量（可以理解为代码编译成的汇编代码的行数）。如果一个回合的计算超出限制，该机器人的进程会被强制结束，并在下回合内从断点处恢复。

可能稍微说的有点多了。当然这还不是全部的规则，更详细的规则可以到 #link("https://releases.battlecode.org/specs/battlecode23/2.0.3/specs.md.html")[官方文档] 中查看。
...
由于地图的大小不会超过60x60，地图上一个点的坐标只用6位`x`坐标+6位`y`坐标=12位二进制即可表示。
...
其中左边是高位，右边是低位。`T`表示具体信息，`X`表示`x`坐标，`Y`表示`y`坐标。
...
将`x`坐标和`y`坐标的存储从7二进制位缩减成了6二进制位。前面的`C`暂时没有分配用途。
...
答：如果机器人无法写入内存，那么它可以先将所有探到的位置记录在自己的内存中（每个机器人有不超过5M的本地内存），等走到增幅器附近再写入。
]
...
后来我们学习了那些和我们对局并在兵力上战胜我们的其他队伍的策略，发现他们的发射器大多都是有组织地行动，一次来很多，而不是一个一个地派过来。#link("https://ttzytt.com/")[TZ] 受此启发，在发射器的代码中加入了自动检测战场的部分。
...
后来我们组的 #link("https://ttzytt.com/")[TZ] 大佬试图使用 A\* 算法来寻路，但是比较不幸地，写挂了。我们也想尝试用 Cooperative A\* 算法，但是因为各个机器人之间共享信息非常不方便，我们组也没有想到比较好的解决方法，这个计划也被搁置了。
