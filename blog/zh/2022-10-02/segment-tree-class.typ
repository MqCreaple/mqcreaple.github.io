// title: 封装一棵线段树
// summary: 用指针封装一棵线段树，支持区间加与区间求和，以及改用 unique_ptr 的改进版。
// tags: algorithm
// category: tech
...
最近得了个“小感冒”，实在懒得去做题，然后就在其他人的博客里瞎逛。无意间发现了 zxp 大佬之前写过的#link("https://zxp2019.github.io/blog/article/segment-tree-class/")[_封装一棵线段树_]，然后就想把这里的代码稍微改得更像 C++ 标准库一点。

= 基本结构

首先肯定是不能用数组来写线段树了，而应当用一个更高级的工具：*指针*。

我们在线段树结点的`struct`里放这些东西：
...
_（为了对应大部分 C++ 标准库的规则，这里的左边界是包含的，而右边界则不包含，相当于这个结点覆盖的是区间上`[left, right)`的部分）_
...
我们用 C++ 新特性：智能指针，来代替上文中的`Node*`。这增加了程序的安全性。
...
（如果你学过 Rust，可能会对`unique_ptr`感到很熟悉）
