// title: 有类型λ演算
// summary: 在λ演算中添加“类型”会有哪些变化？
// tags: lambda-calculus, computation
// category: tech

#import "../../template.typ": article, appendix
...
#quote(block: true)[
前注：其实这篇文章在2022年9月就写好了，当时本来打算和前几篇讲 Lambda 演算的文章一起发，结果在硬盘里一放就是三年，今天才翻出来。正好最近看了些关于直觉逻辑主义的内容，里面也有一些关于类型系统的内容。在阅读完这篇3年前的文章，修改了几处错误并添加了部分内容后，将其重新发布。
]

*温馨提示：如果你不了解λ演算，请先阅读#link("/zh/posts/2022-08-27/lambda/")[这篇文章]和#link("/zh/posts/2022-09-02/y-combinator/")[这篇文章]以获得最佳食用体验*

_注：本文所有的小写字母\/单词是变量，大写字母\/首字母大写的单词是类型。为了不和类型混淆，本文中“真”用`true`而非`T`，“假”用`false`而非`F`_
...
$ f := lambda a^("Int"). thin lambda b^("Int"). thin a $

= 函数类型

既然λ表达式里所有对象都有类型，那么函数也不例外。

我们定义，如果函数$f$仅输入一个参数，并且参数的类型为$U$，返回值的类型为$V$，那么函数$f$的类型为：$U -> V$。

例如，逻辑非（`NOT`, $not$），表达式如下：
...
这时候就可以用到之前提到过的_柯里化_。如果你忘记了函数的柯里化，可以回到#link("/zh/posts/2022-08-27/lambda/#多元函数")[这里]。

我们将$g$看作一个关于$a$的单值函数，它的返回值是另一个类型为 $U -> V$ 的λ表达式。也就是说，$g$的类型是：
...
*注意：`→`不具有结合律，`(T→U)→V`和`T→(U→V)`是不同的类型。前者表示“输入类型为`T→U`的λ表达式，输出类型为`V`的值”，而后者表示“输入两个类型分别为`T`和`U`的值，输出类型为`V`的值”。比如，当`T`、`U`和`V`均为实数$RR$的时候，$(RR -> RR) -> RR$ 表示的是一元泛函，而 $RR -> (RR -> RR)$（通常数学上也写成 $RR times RR -> RR$）表示的是二元函数。*

类似的规则可以拓展到更多元的函数。如果你下次遇到了一个一连串箭头的类型表达式：

$ T_1 -> T_2 -> T_3 -> dots.c -> T_n -> U $
...
你会发现，这个表达式里的$x$不可能拥有确定的类型。假如$x$的类型是$T$，那么为了让函数调用$(x thin x)$合法，$x$的类型又必须输入值为$T$的函数，而一个类型显然不能包含它自己，矛盾。
...
接下来将会定义一系列对有类型λ演算的扩展运算，我们称其为_F系统（System F）_。
...
我们希望不管`a`和`b`是什么类型，`f`都有定义。为了实现这个语法，我们引入$Lambda$（大写 Lambda）记号：
...
我们记函数`f`的类型为：`∀T. T→T→T`，因为这个函数接收一个类型`T`，两个类型为`T`的参数，并且返回类型是`T`。在向`f`中代入一个类型之后，`∀`会被替换成具体的类型，比如：
...
这个概念与许多计算机语言中的函数泛型有点像。比如 C++ 中可以用模板函数实现与上述函数`f`相同的效果：
...
#table(
  columns: 3,
  [类型名称], [类型表达式], [举例],
  [布尔类型], [`∀T. T→T→T`], [`true` = `ΛT. λa:T. λb:T. a` \
`false` = `ΛT. λa:T. λb:T. b`],
  [自然数类型], [`∀T. T→(T→T)→T`], [`0` = `ΛT. λz:T. λf:(T→T). z` \
`1` = `ΛT. λz:T. λf:(T→T). (f z)` \
`2` = `ΛT. λz:T. λf:(T→T). (f (f z))` \
...],
  [元素为`U`的列表类型], [`∀T. T→(U→T)→T`], [空列表 = `ΛT. λnil:T. λcon:(Int→T). nil` \
[1, 2, 3] = `ΛT. λnil:T. λcon:(Int→T). (con 1 (con 2 (con 3 nil)))`],
  [叶节点元素为`U`的二叉树类型], [`∀T. (U→T)→(T→T→T)→T`], [#image("binary-tree-example.png", width: 50%)#linebreak() `= ∀T. λl:(Int→T). λn:(T→T→T). (n (n (l 1) (l 2)) (l 3))`],
)

讲了这么多关于 Lambda 表达式的内容，可能你还有点疑惑：Lambda 表达式的这些数据类型都是怎么做出来的？数学家们都是怎么想到诸如`∀T. T→T→T`可以表示布尔类型，`∀T. T→(T→T)→T`可以表示自然数？

细心的你可能已经从上面这些例子中看出来了，这些类型的创建都是有规律的。我们不妨对比一下这些类型的 Haskell 实现：
...
练习：你能将以下 Haskell 类型\/函数写成λ演算的类型形式吗：
