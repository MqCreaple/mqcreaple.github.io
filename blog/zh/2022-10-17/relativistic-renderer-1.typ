...
// summary: 用四维向量存储惯性参考系与光线，以球体为例推导光线求交。
...
首先，我们可以先选定一个参考系，称为_世界参考系_。这样其他一切惯性参考系都可以用它们和世界参考系之间的相对速度 $arrow(u)$ 来表示了。

但是注意，只记录一个速度向量 $arrow(u)$ 是不够的，因为惯性系之间不只有相对速度，还有可能有平移、旋转之类的相对变换。

这里我们只考虑相对速度 $arrow(u)$ 和平移 $mathbf(p)$ 这两个因素，暂时不考虑旋转#strike[，主要是因为我懒得推旋转参考系的公式]。也就是这样的一个类：
...
显然，世界参考系就是平移 $mathbf(p)$ 和速度 $arrow(u)$ 都为零向量的参考系。
...
具体的公式可以参考#link("/zh/posts/2022-10-14/relativistic-renderer-0/")[上一篇博客]。
...
为了唯一确定一条这样的射线，我们需要光的*光源 $mathbf(r)$*和光线*方向 $arrow(d)$*。其中 $arrow(d)$ 一定是一个单位向量。
...
#figure(
  image("ray-spacetime-diagram-0.png", width: 70%),
  caption: [#strike[众所周知 geogebra 啥都能画]],
)
...
光线的参考系变换也很简单，只需要对光源 $mathbf(r)$ 做事件的参考系变换，方向 $arrow(d)$ 做速度的参考系变换即可。
...
#quote(block: true)[
`std::optional`是C++17的新特性，可以用来表示一个“有值或者没有值”的对象。更多信息可以看#link("https://en.cppreference.com/w/cpp/utility/optional")[cppreference]。
]

另一个值得注意的点是：由于光线追踪的过程是*回溯*光的路径，所以每次求出的交点应该在光路的*反方向*上，`distance`值应当为负数。

#image("forward-ray-tracing-0.jpg", width: 70%)

#quote(block: true)[
实际上由于光路可逆原则，将光线的方向全部反向之后，每次求光路正方向上的交点，理论上也是可行的。

*但是*，如果你真的打算这么写，那么就要尤其注意光线的参考系变换。因为这样的话，`Ray`类里记录的方向向量 $arrow(d)$ 和光的速度向量*是相反的*，光线的`toLocal`和`toWorld`里对应项也需要变符号。

#strike[之前被这个 bug 困扰了一个晚上，在这里写下来以警示后人]
]
...
假设有一个在*物体参考系*里的光线`ray`（不是世界参考系，所以不用考虑参考系变换）。光线从原点 $arrow(r)$ 出发，向前走了距离 $l$，走到的坐标是：

$ arrow(r)_1 = arrow(r) + l arrow(d) $

如果 $arrow(r)_1$ 是光线和球的交点，那么 $arrow(r)_1$ 到球心（即原点）的距离一定等于球的半径，即：

$ ||arrow(r)_1|| = ||arrow(r) + l arrow(d)|| = R $

#image("ray-sphere-intersection-0.png", width: 70%)

将向量模长写成向量点乘自己，也就是：

$ (arrow(r) + l arrow(d)) dot (arrow(r) + l arrow(d)) = R^2 $

展开这个式子：

$ (arrow(r) + l arrow(d)) dot (arrow(r) + l arrow(d)) = arrow(r) dot arrow(r) + 2 l arrow(r) dot arrow(d) + l^2 arrow(d) dot arrow(d) = R^2 $

$ arrow(d)^2 l^2 + 2 (arrow(r) dot arrow(d)) l + (arrow(r)^2 - R^2) = 0 $

由于 $arrow(d)$ 是单位向量，$arrow(d)^2 = 1$，代入原式得到：

$ l^2 + 2 (arrow(r) dot arrow(d)) l + (arrow(r)^2 - R^2) = 0 $
