...
快速平方根倒数算法，最早见于1999年的3D游戏 Quake III Arena 的源代码中。该算法一经公布，立刻因它的巧妙而被广为流传。
...
假设现在你有一个二进制小数，可是计算机只认识0和1组成的二进制串，并不认识小数，你该怎么办呢？
...
最后，为了能够表示负数，需要单独拿出来一位二进制作为*符号位*（0为正，1为负）。我们得到了浮点数的 IEEE 754 规则：
...
第10到32位是有效数字位 $M$，共23位，运算时需要在前面脑补一位$1$和小数点。
...
#image("q-rsqrt-1.jpg", width: 70%)

现在，我们作一条 $A$ 点上的切线，切线与横轴的交点作为第一轮迭代的结果。

#image("q-rsqrt-2.jpg", width: 70%)

可以看到迭代后的点离函数的根已经近了很多。

#image("q-rsqrt-3.jpg", width: 70%)
