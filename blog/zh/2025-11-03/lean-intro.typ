// title: Lean 简介：基础程序证明
// summary: 从函数、类型到证明策略的 Lean 入门。
...
本文主要面向没有接触过 Lean 的初学者，将 Lean 的各个功能以一种更容易理解的顺序呈现给读者。建议读者具有以下前置知识后再阅读：

+ 函数式语言（了解基础概念即可）
+ 基础群论知识

本文全文内容均来自 The Lean Language Reference @leanReference，有能力者也可以尝试阅读原文。
...
- 纯函数是指返回值永远确定、无副作用、且对于同样的输入永远返回同样输出的函数。比如下面这个 Python 语言写的函数就是一个纯函数：
...
- 常数和变量被定义为无参数的函数。由于纯函数的要求，变量被赋值之后无法更改，只能被覆盖。如果你了解 C 语言，可以理解成函数式语言里所有变量都是 C 里的 `const` 修饰符修饰的。
- 函数可以作为参数传入其他的函数。即：可以定义“函数的函数”、“函数的函数的函数”，以此类推。
- 函数可以*柯里化（Currying）*，即当某个函数的参数不完整时，它会被认为是接收剩余参数的函数。

  举个例子：如果 $f: RR times RR -> RR$ 是一个接收两个实数的二元函数，那么 $(f thin 1)$ 是一个一元函数，接收一个实数 $x$，返回 $(f thin 1 thin x)$ 的值。

  举个更具体的例子，加法操作（$+$）可以看作是一个二元函数。那么对加法使用柯里化，$(+ thin 1)$ 就表示一个一元函数 $f(x) = 1 + x$。

关于λ演算的更多内容，可以看#link("/zh/posts/2022-08-27/lambda/")[这篇文章]。那么说完基础知识，我们再来讲 Lean 语言的特性。

= 函数：`def`、`theorem`、`abbrev` 和 `fun`

首先就是怎么在 Lean 里定义函数。Lean 提供了三种定义函数的关键字，分别是 `def`、`theorem` 和 `abbrev`，用以区分不同的函数类型。
...
类型也可以作为普通参数传进来。并且由于 Lean 完全兼容依赖类型（如果你忘了依赖类型是什么可以看#link("/zh/posts/2025-10-21/logic-and-proof/")[这里]），我们可以写出这样的函数：
...
上述代码的右箭头符号 `→` 可以在 VSCode 里用 `\to` 或者 `\r` 再按空格打出来。或者也可以用 ASCII 字符组合 `->` 替代。

在 Lean 中也可以使用递归函数，即在函数中调用自身。但是 Lean 要求所有递归函数都必须证明其会在有限层内终止。这一点我们会在 @sec:by 中详细说明。

`def` 的变体 `abbrev` 和 `theorem` 在语法上和 `def` 没有区别，只要把 `def` 关键字换成 `abbrev` 或者 `theorem` 即可。从习惯上来说，`abbrev` 通常用于定义某个较长的名称的简写，而 `theorem` 通常用于定义某个定理。根据#link("/zh/posts/2025-10-21/logic-and-proof/")[上期]提到过的丘奇-霍华德同构，定理和函数类型是一一对应的，这也是为什么我们能用定义函数的语法来定义定理。

编译器也会给这两种函数做不同的处理：

- 所有用 `abbrev` 定义的函数都必须是#link("https://lean-lang.org/doc/reference/latest/Definitions/Recursive-Definitions/#--tech-term-Reducible")[强归约]的。即：对于 `abbrev X := Y`，所有出现 `X` 的地方都会被替换成 `Y`。
...
某些场合下我们可能想要传入一个函数，但不想再给这个函数一个全局变量名。这时候使用 `fun` 定义匿名函数就会很方便。比如这样：
...
这里 `fun x` 后面这个箭头可以用 `\map` 打出来。或者也可以用 ASCII 字符组合 `=>` 替代。
...
Lean 包含一些内置的数据类型，比如自然数 `Nat`、布尔类型 `Bool`、有限自然数类型 `Fin`、以及 `Int32`、`Int64` 等类型。而“类型”本身也是一个类型，在 Lean 中记作 `Type`。你可以将一个类型赋给一个变量，像这样：
...
同样的，`Type` 本身也有一个类型，在 Lean 中记作 `Type 1`，而 `Type 1` 的类型为 `Type 2`，以此类推。
...
Lean 还有一个特殊的类型：`Prop`。`Prop` 用来表示所有命题的类型，而 `Prop` 本身的类型是 `Type`。如下所示：
...
而#link("/zh/posts/2025-10-21/logic-and-proof/")[上篇文章]说过，命题本身就是类型，也可以作为函数返回值。不仅如此，所有 `theorem` 标记的定理的返回值必须是 `Prop` 类型的。
...
Lean 为我们定义好了很多常用的类型函数，比如用于表示函数类型的 `→` 就可以看作是输入两个类型、输出一个类型的函数（当然这个符号的定义是编译器内置的，否则你连函数都写不出来）。

== 组合类型：`structure`

正如大多数语言里对 `struct` 和 `class` 的定义一样，Lean 里的 `structure` 可以用来表示多个类型的组合。例如这样：
...
递归类型有点像 Haskell 里的 `data` 和 Rust 里的 `enum`（或者准确来说 Rust 的 `enum` 最早就是从函数式语言里抄过来的），它允许该类型的成员选择几种不同可能性中的一种。像这样：
...
有了这些基础知识，我们就可以看一下 Lean 的标准库里是怎么实现各种不同类型的了。
...
`Bool` 和 `Nat` 两个类型都是通过 `inductive` 定义出来的。`Bool` 很简单，要么真要么假，因此在 `inductive` 的两个分支里放上 `false` 和 `true` 即可。自然数 `Nat` 的定义略微复杂一些：自然数有可能是0，也有可能是某个其他自然数的后继，因此两个分支分别是 `zero` 和 `succ (n)`。关于为什么要这样定义，也可以看#link("/zh/posts/2025-09-03/typed-lambda/")[这篇文章]。

Lean 还定义了一些类型函数，比如 `Option`、`List` 等。这些类型本身接收另一个类型 `α` 作为函数，返回一个类型。
...
`Commutative` 是一个类型函数，为类型 `α` 绑定了交换律，即 `add a b = add b a`，而我们想要证明 `(add (add a b) c) = (add c (add b a))`。可以看到，为了书写这个证明，我们不得不创建一个 `instComm` 对象，并在所有需要加法的地方使用 `instComm.add`，导致整个证明写出来十分啰嗦。而如果将 `structure` 换成 `class`，同样的证明就可以非常简单：
...
首先，我们希望将 `Group` 定义为一个 Type Class。这样方便让别的类型去实现群的性质。
...
不妨以旋转群 Z₃为例。这个群只包含3个元素：不旋转、往左旋转120°、往右旋转120°。我们可以这样表示：
...
此处我们先将结合律的证明留空#strike[读者自证不难]。这样，我们就算是为 `Z₃` 实现了群的所有性质了。
...
但很可惜，Lean 看不懂我们这段文字描述，你必须将这段文字转化成一段 Lean 函数，同时确保函数上的参数符合 Lean 类型检查规则。我们不妨先来看一下有哪些定理可以用：
...
如果你在 VSCode 上安装了 Lean 插件，可以将光标移到 `sorry` 之前，查看目标表达式：
...
Lean 的检查结果表明该代码没有类型问题。自此，证明结束。完整证明如下：
...
显然，这样的证明太繁琐、太烧脑了。我们也不想每次证明的时候都事无巨细地从头推导一遍。这样的效率太低了。因此，Lean 引入了*证明策略*这一功能。它使用类似 Monad 的方法，让用户可以用类似指令式的方法来做公式推导。其格式大致如下：
...
这样的表达式就简洁易懂多了。可以理解成 `rw` 在内部帮你写好了一系列 `Eq.subst` 函数的调用，实现了表达式替换的效果。实际上所有的证明策略都可以用 Lean 的原生语法实现。

Lean 还提供了一些语法糖，比如可以用 `rw [← a=b]` 来表示 `rw [b=a]`。因此，上面的证明还可以更简化：
...
$ forall a, b in G, thin a^(-1) = b^(-1) -> a = b $
...
也就是说，我们需要返回一个函数类型，输入一个 `a⁻¹ = b⁻¹` 的证明，输出一个 `a=b` 的证明。如果不使用 Lean 的证明策略，那么我们就需要在结果处写一个匿名函数：
...
接下来我们可以引入第二个中间变量：`h2 : a * (b * b⁻¹)`。别忘了乘号是从左往右结合的，如果不加这一步的话 Lean 是没法直接直接计算 `(b * b⁻¹)` 的。
...
Lean 提供了非常多的证明策略。除了以上的三条之外，还有很多常用的证明策略没有详细展开说明。下面几个章节里我简要列举一些其他的常用策略，并给出它们对应的不用策略的表达式。以下列举出一些常用的证明策略，更多内容也可以看#link("https://lean-lang.org/doc/reference/latest/Tactic-Proofs/#tactics")[Lean Language Reference]。
...
如果 `apply` 的函数有多个参数，比如 `h : p → q → r`，那么 Lean 就会生成两个子目标 `p` 和 `q`。如下所示：
...
+ 抽象代数
  ...
  + 请尝试实现一个类型来表示*同态映射*。对于群 $(G, ast)$ 和 $(H, diamond)$，函数 $phi: G -> H$ 在满足以下条件时被称为群 $G$ 到 $H$ 的同态映射：

    $ forall u, v in G, thin phi(u ast v) = phi(u) diamond phi(v) $

    同时请尝试证明以下定理：

    - 同态映射会将 $G$ 的单位元 $1_G$ 映射到 $H$ 的单位元 $1_H$。
    - 同态映射的复合仍然是同态映射
  + 请在群同态映射的基础上定义群的*核*（所有 $G$ 中被映射到 $1_H$ 的元素构成的集合）。并尝试证明 $op("ker") G$ 是群 $G$ 的一个子群。
  + 请尝试定义环（Ring）和域（Field）结构，使用继承关系让其兼容已经写完的群的定义。
+ 数论

  + 请尝试阅读 Lean 标准库对于自然数 `Nat` 的定义。其中包含了许多和自然数的数论性质相关的证明。请选择一两个你感兴趣的证明并尝试理解证明过程。
  + 请写一个 `Nat → Prop` 的命题函数来判断某个自然数是否为质数，并尝试证明以下定理：
