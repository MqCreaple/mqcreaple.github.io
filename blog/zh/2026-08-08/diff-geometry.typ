// title: 微分几何1：微分流形
// summary: 微分流形的定义、图卡与图集。
// tags: mathematics, differential-geometry
// category: tech

#import "../../template.typ": article, mathbf, three-js-figure, theorem, definition, proof
...
#figure(
  html.frame[
    #cetz.canvas({
      ...
      content((6.5, 2.5), [$phi_beta #sym.circle phi_alpha^(-1)$])
      content((6.5, 0.5), [$phi_alpha #sym.circle phi_beta^(-1)$])
    })
  ],
  caption: [两个坐标卡 $(U_alpha, phi_alpha)$ 与 $(U_beta, phi_beta)$ 之间的换算函数为 $phi_beta circle phi_alpha^(-1)$ 或者 $phi_alpha circle phi_beta^(-1)$，有时也简写为$phi_(beta alpha)$和$phi_(alpha beta)$。]
...
#definition[
  ...
  3. 对于任意点 $p in M$ 的开邻域 $p in U in cal(O)$，若存在 $alpha, beta$ 使得 $U subset.eq U_alpha$ 且 $U subset.eq U_beta$，则函数 $phi_beta circle phi_alpha^(-1)$ 是一个光滑函数（*$C^infinity$兼容*）。 
] <def:manifold>
...
#definition[
  ...
  $ c_1 lr(dif / (dif t) (phi_alpha circle gamma_1)(t) |)_(t = 0) + c_2 lr(dif / (dif t) (phi_alpha circle gamma_2)(t) |)_(t = 0) = lr(dif / (dif t) (phi_alpha circle gamma_3)(t) |)_(t = 0) $
...
#definition[
  ...
  1. $ p in U, f(p) in V $
  2. $(psi circle f circle phi^(-1))$ 是一个光滑函数。

  则称映射$f$*光滑*。
] <def:smooth>

由于 $(psi circle f circle phi^(-1))$ 的定义域和值域为欧氏空间 $RR^m$ 和 $RR^n$ 的子集，这个函数的光滑性是已经有定义的。这个定义其实同样是把一般流形的性质用坐标卡变成我们熟悉的欧氏空间中的性质。

使用与 @thm:equiv-independent-from-chart 相同的方法，不难证明将 @def:smooth 的“存在”换成“任意”仍然成立，因为表达式 $(psi circle f circle phi^(-1))$ 的左右两侧都可以复合上任何坐标变换函数。
...
至于为什么 $M$ 上的路径等价类 $[gamma]$ 经过映射之后仍然是流形 $N$ 上的等价类，这一点可以通过 $(psi circle f circle phi^(-1))$ 的雅可比矩阵来证明。具体证明过程在此省略了。同时也可以用雅可比矩阵来说明 $dif f$ 是一个线性函数。
