...
之前写过许多篇关于λ演算和函数式编程的文章 #link("/zh/posts/2022-08-27/lambda/")[\*] #link("/zh/posts/2022-09-02/y-combinator/")[\*] #link("/zh/posts/2025-09-03/typed-lambda/")[\*] #link("/zh/posts/2025-10-21/logic-and-proof/")[\*]。在这里，我想以 Lean 为例，展示一下一个简单的证明是如何一步步变成λ演算最底层的代码的。
...
我们使用 `exists` 策略来给出 `∃ x : Nat, x + 1 = 2` 的一个例子，即 `x = 1`。接下来，Lean 会自动为我们计算1+1的值并证明其等于2。整个证明只用一行就能写完。但是，就在这一行的背后是许多被省略掉的细节。
...
接下来我们还需要定义 `∃` 类型。#link("/zh/posts/2025-10-21/logic-and-proof/")[这篇文章]里说过，存在谓词对应着类型理论中的Σ类型，因此我们可以这样定义：
...
事实上，Lean 语言里形如 `∃ x : Nat, x + 1 = 2` 这样的命题最终会被宏编译成 `Exists (fun x : Nat => x + 1 = 2)` 这样的形式，也就是和我们刚刚定义的这个函数几乎完全一致。
...
theorem one_plus_one_is_two
  ...
  := _Exists.intro _one (_NatEq.rfl _two)
```

现在这个类型表达式已经长到有点不太能读了。但我们的证明仍然很简单，即：用 `_Exists.intro` 去取 $x=1$，并提供 $1+1=2$ 的证明。由于2是1的后继，证明 $1+1=2$ 这部分我们直接使用 `_NatEq.rfl _two` 即可。编译器会自动展开 `_Nat.add` 函数的定义，在两边都得到整数2，从而判定两边的式子是等价的。

== Step 2：转写成 $lambda^C$ 系统

上面的所有定义仍然是用 Lean 语言书写的。为了完全剥离编程语言层面的细节，我们不妨用纯数学的 $lambda^C$ 系统来重新书写上面这段代码。

首先就是要重写 `inductive` 关键字定义的归纳类型。#link("/zh/posts/2025-09-03/typed-lambda/")[这篇文章]里简要提到过怎么在λ演算中构建归纳类型#strike[然后我现在才知道我总结出来的这套东西有名字，叫#link("https://en.wikipedia.org/wiki/Mogensen%E2%80%93Scott_encoding")[摩根森-斯科特编码]]。比如，自然数类型
...
这里我使用一些像 `zero`、`succ` 这样的多字符变量名。虽然也可以全部改成 `z`、`s` 这样的单字符名，但这样的话代码可读性会差很多。因此，我选择将λ演算中与前文 Lean 代码里对应的部分使用同一个变量名，以示二者之间的联系。

这里我们使用的符号和#link("/zh/posts/2025-09-03/typed-lambda/")[上一篇文章]中的符号略有不同。我们统一使用 `Π` 来标记一切依赖类型，不论依赖类型的参数是数值还是类型本身。`Πα:*` 表示这个函数接受一个类型作为参数，也就是上篇文章中用的 `∀α` 符号。读者可以尝试使用 $lambda^C$ 的类型推断规则来尝试证明一下 `_Nat.zero` 的类型是 `_Nat`，而 `_Nat.succ` 类型是 `_Nat→_Nat`。

#quote(block: true)[
参考：类型推导规则

$ (Gamma tack.r m : (Pi_(x:sigma). thin tau) quad quad Gamma tack.r n : sigma) / (Gamma tack.r (m thin n) : tau[n/x]) $

$ (Gamma, thin x:sigma tack.r m : tau) / (Gamma tack.r (lambda x:sigma. thin m) : (Pi_(x:sigma). thin tau)) $

其中 $a[b/c]$ 表示在 $a$ 表达式中用 $b$ 符号去替换所有 $c$ 符号。

以及一个类型简写规则：若 $tau$ 类型不依赖 $sigma$ 类型，（即：$tau$ 的表达式中没有任何项的类型是 $sigma$），则：

$ (sigma -> tau) := Pi_(x:sigma) tau $
...
如果你忘了怎么在λ演算里去掉函数递归，可以看之前的#link("/zh/posts/2022-09-02/y-combinator/")[这篇文章]。总之就是可以通过某些神奇方法将右侧表达式的递归调用变成非递归调用。

类似地，我们可以写出 `_NatEq` 的表达式。`_NatEq` 类型都不像 `_Nat` 那样是个单纯的类型，而是依赖于某些变量的类型函数。但转写成λ演算的过程也大差不差。先把之前的 Lean 代码粘贴一下：
...
#strike[这一整段代码我都是手动检查类型的，感觉脑子已经不够用了]
...
看起来很恐怖，但实际上仔细观察的话会发现其中有很多重复部分，所以这段表达式的实际信息量没有写出来的这么大。而且显然，编程语言在做类型检查时不会把表达式这样完全展开，只有用到特定部分的时候才会展开内部。另外，函数式语言也不会把这些表达式完全按照最底层的λ演算的形式写出来，而是会把某些较小的数据类型优化成更容易让 CPU 计算的形式，比如用二进制编码整数——毕竟这里只是证明 $1+1=2$，万一要证明 $289+1378=1667$，编译器也不可能展开1667层函数嵌套吧。

λ演算最早被提出时，是作为一种计算能力等价于图灵机的计算系统，而高级函数式语言可以看作是在λ演算这个“机器码”上建立的“高级语言”。由于冯诺依曼架构在工程上更容易实现，现在的（几乎）所有计算设备都是基于冯诺依曼架构的“加强版”图灵机。如果我们有一台假想的机器能够把λ演算像机器码一样执行，那么几乎所有的数学证明都可以用这台“λ演算机”来做检验。
...
可以参考 Lean 标准库中的 #link("https://github.com/leanprover/lean4/blob/a0e742be5ef1f31c99b1dd0d9b73a0b4cf6b86fc/src/Init/Classical.lean#L29")[choose] 函数
]
...
+ 任何满射 $f : M -> N$ 都存在逆映射。
+ 任何向量空间都有一组基向量。
+ （Vitali 集）存在一个 $RR$ 的不可测子集。

从上面的例子可以看出，直觉主义逻辑又有必须使用构造性证明、缺少排中律等等限制，在某些情况下可能不能证明某些经典逻辑下正确的结论。

= 形式化证明工具概述

从数学证明的过程提炼出几条简单的逻辑规则，并使用这些逻辑规则去代替自然语言来书写数学证明，这就是#link("https://en.wikipedia.org/wiki/Formalism_(philosophy_of_mathematics)")[数学形式主义]。形式主义在生活中可能是个贬义词，但在数学里代表着最严格的证明方式。通过选择恰当的公理体系，我们就可以将数学证明形式化为字符串的变换，从而使用计算机来处理。

某些证明工具可以全自动在符号空间中搜索某个定理的证明，而不像 Lean 一样只能用来验证某个证明的正确性。因此，前者被称为*自动定理证明器*，而后者被称为*证明辅助器*。通常来说，前者由于基于一阶逻辑以及等效的逻辑系统，不用处理过于复杂的命题，因此其状态空间小到可以用搜索算法来自动化搜索证明；而后者一般基于二阶或更高阶逻辑（比如 Lean 从理论上支持无穷阶逻辑，只是一般我们只用到二阶），因此状态空间复杂到几乎无法用程序来枚举，因此才需要人为引导来书写证明。自动定理证明通常用于证明一些逻辑较简单的计算机程序，确保程序在任何输入下都能输出符合预期的结果；而证明辅助器则更多用于证明逻辑较复杂的数学命题。

当然，根据哥德尔不完备性定理，任何形式化体系里都会有无法证明的定理，没有任何形式主义是万能的。当然，对于实践中会用到的数学定理，现有的定理证明工具已经够用了，因此也不用太担心不完备性的问题。

关于定理证明工具更多信息，可以咨询大模型（因为别的我也不太了解了 lol）。