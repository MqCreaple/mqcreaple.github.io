...
#image("MT0000.jpg", width: 70%)

#image("MT0001.jpg", width: 70%)
...
#image("MT0002.jpg", width: 70%)

学过一点微积分的同学们应该知道，其实某一个点附近的坡度，就是该点的导数 $(dif f) / (dif x)$，而极大、极小值点，就等价于该点附近导数为0，即

$ (dif f) / (dif x) = 0 $

当然，这个规律也可以推广到多维函数，只不过我们不再使用导数，而是*偏导数*。

#image("cmich.edu-pderv1.gif", width: 70%)
...
#image("MT0004.jpg", width: 70%)

多元函数中这样的例子则就更多了。比如，在两个山峰之间存在着一个“鞍点”，见下图：

#image("MT0005.jpg", width: 70%)

这个点从 $x$ 方向上看是极小值点，而从 $y$ 方向上看是极大值点，总不能说它既是极大值又是极小值吧！因此这个点虽然梯度为0，但并不是极值点。

它的梯度场形状如下：

#image("MT0006.jpg", width: 70%)
