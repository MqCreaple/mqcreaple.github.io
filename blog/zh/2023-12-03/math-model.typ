// title: 浅谈数学建模
// summary: 数学建模，难以同时做到“准确”和“易懂”。
// tags: mathematics, math-modeling
// category: tech
...
上个月刚参加了今年的 HiMCM 比赛（High School Mathematical Contest in Modeling），苦肝了两周之后也算是写出了一个比较满意的 paper，这里也顺便写写自己的感受和收获。

#strike[本来打算比赛结束之后就写的，但一直拖到12月，我也是够懒的]
...
同时，数学建模竞赛也会要求参赛者有较强的计算机水平。常用的软件包括 Matlab，Mathematica，python 等等。当然，和 OI 竞赛不同，数学建模竞赛不太需要你去了解某个算法背后的原理，而是知道怎么去“用”某个算法。你需要知道哪些工具的哪些功能可以用于解决哪些问题，而不用太关注到底这些功能背后的原理是什么样的。

最后一点就是，数学建模对论文阅读和写作能力还是有很高的要求的。这方面我可能没太多话语权#strike[因为我写论文写得很烂]因为我主要负责的是数学和计算机。但是，写学术论文的必备工具（比如 LaTeX）和阅读论文的能力还是需要强加练习的。你在建模比赛的两周里可能会读10到50篇论文不等，你需要迅速查找论文并提取论文中的有效信息。

既然是竞赛，那么我肯定是要锐评一下的（（

研究纯数学的理科生们可能会说：“数学竞赛不就是一个文科比赛吗？”这个质疑也不无道理。相比起更常见的学科竞赛，数学建模确实在评判的时候有更多主观因素，这也给黑箱操作留了更多空间（当然这也跟不同比赛的评委有关系，可能国内的数模竞赛会好些#super[？？]）。比如某年的某个 HiMCM 最高奖获得者，他的论文甚至就是用 desmos 把题目给的数据拟合了一遍（我甚至怀疑有可能是手动调参的），就完全没有任何技术含量。

#image("himcm-screenshot-3.png", width: 70%)

也不是说数学建模竞赛就都很水或者都很简单。我个人认为对刚开始接触应用数学的同学来说，数学建模竞赛是一个挺好的学习和锻炼的机会。在准备数学建模竞赛的过程中你会收获很多在教科书上很难接触到的实用技能。

但话说回来，*数模竞赛和真实的科研还是有很大区别的*。首先就是大部分数模竞赛都会有一个1到2周的限时，而这对于真实的科研来说是远远不够的。我们队在今年的 HiMCM 里面就能明显感觉到时间真的很不够用，即使是在爆肝了两周、每周末几乎把所有时间全都倾注到这个比赛上，最终也没能把我们的所有想法全都在论文上呈现出来。

还有就是，数学建模竞赛本身就决定了参赛者没法完全不功利。如果你想在竞赛里面得高分，你可能需要用更多更高深的数学模型和算法，但不是所有“高级方法”在所有问题上都适用的。最终的结果大概率就是那些得奖的论文为了追求“创新”和“独特”使用了一大堆非常 fancy 的技巧，但是模型出来的结果可不一定比别的方法好。
...
1. 像游戏《都市天际线》那样，给每一辆车一个尽可能精确的行为逻辑，然后在城市地图上放几千辆车来看最终的模拟结果。
   ...
   #image("cities-skylines-Traffic_Info_View_SS.png", width: 70%)
2. 只考虑某一路段的一定数量的车，假定这段路被分成了许多离散的格点，并且每辆车在每个时刻都只能在某一个格点上。定义一套格点状态演化的规则，观察在不同的规则下会有什么样的结果。

   #image("modeling-traffic-velocity-map-130.png", width: 70%)
...
#figure(
  image("modeling-traffic-velocity-map-45-0.2.png", width: 70%),
  caption: [200路长，42辆车，每辆车每个时刻有0.2的概率减速],
)

可以看到上述图像中有一处堵车随着时间推移就逐渐消失了。

#figure(
  image("modeling-traffic-velocity-map-45-0.png", width: 70%),
  caption: [200路长，42辆车，每辆车每个时刻都严格按照当前时刻的最大速度行驶],
)
