// title: 浅谈并行计算
// summary: 并行计算的基本思想、“串行”和“并行”的复杂度分析、分治树。
...
它将数组从中间分成了两段，然后创建了两个线程来分别计算前半段和后半段。如果你的 CPU 有至少两个核，那么`calcSum2`计算同样长的一段数组所需的时间将是`calcSum`的一半。
...
我们可以先引入一个_简化版模型_。每个任务的代码可以被看作是由两部分组成：*串行部分*和*并行部分*。任务的并行部分能够被分割成数个相互独立的子任务，而串行部分则必须在单一计算机上执行。

#image("parallel-0.png", width: 70%)
...
并行计算不仅减少了计算时间，还有可能降低算法的时间复杂度。完全串行的情况下，时间复杂度等于_并行部分的时间复杂度_+_串行部分的时间复杂度_；而在理想并行的情况下，时间复杂度仅为_并行部分的时间复杂度_。如果随着数据规模$N$的提升，并行部分的时间复杂度提升的没有串行部分快，那么这样做就可以降低时间复杂度了。
...
以下为`mergeSort`一个长度为7的数组时，函数调用的树形结构：
  ...
  #diagram(
    ...
    edge((0, 0), (-2.1, 1.2)),
    edge((0, 0), (2.1, 1.2)),
    edge((-2.1, 1.2), (-3.3, 2.4)),
    edge((-2.1, 1.2), (-1.2, 2.4)),
    edge((2.1, 1.2), (1.2, 2.4)),
    edge((2.1, 1.2), (3.3, 2.4)),
    edge((-3.3, 2.4), (-3.9, 3.6)),
    edge((-3.3, 2.4), (-2.7, 3.6)),
    edge((-1.2, 2.4), (-1.5, 3.6)),
    edge((-1.2, 2.4), (-0.6, 3.6)),
    edge((1.2, 2.4), (0.6, 3.6)),
    edge((1.2, 2.4), (1.5, 3.6)),
  )
]
...
#figure(
  image("parallel-1.png", width: 70%),
  caption: [#strike[如果您有强迫症，请务必原谅我糟糕的绘画水平]],
)

按照图中数字标记的顺序，每次将当前层的所有函数并行执行即可。

接下来做复杂度分析。已知合并两个区间的时间复杂度为$O(N)$，其中$N$为待处理区间的长度，那么_完全串行_时，时间复杂度 $T(N)$ 满足：
