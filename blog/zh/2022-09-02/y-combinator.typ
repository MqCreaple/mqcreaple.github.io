...
// summary: Y 组合子与递归，用 Church 编码在λ演算中构造列表及其运算。
...
*温馨提示：如果你不了解λ演算，请阅读#link("/zh/posts/2022-08-27/lambda/")[上一篇博客]以获得最佳食用体验*

_注：本文的λ表达式中所有出现的下划线`_`，都表示“这个值我懒得算且不关心”_
...
这个表达式很有意思。当你把后面的表达式代入前面的`x`中时，你又会得到和原来一模一样的表达式，你永远也无法化简它。也就是，它是*不可求值*的。
...
由于它和函数的不动点 $x = f(x)$ 长得很像，所以 Y 组合子也叫*不动点组合子（fixed point combinator）*。

_注：Y 组合子并不是唯一的不动点组合子。比如由 Alan Turing 给出的 Θ 组合子_
...
其中`n`为任意有限的自然数。这个式子我们在之后还会用到。如果你忘记了自然数的 Church 计数法，可以回到#link("/zh/posts/2022-08-27/lambda/")[上一篇博客]。

= 函数递归

Y 组合子的一大用途，就是在λ代数中表示“递归”。
...
可惜的是，λ演算里并没有递归的语法。但是有了 Y 组合子，我们也可以实现类似递归的效果。
...
(list5 a b c d e) := λNIL. (pair a (pair b (pair c (pair d (pair e NIL)))))
```
...
`car`和`cdr`这两个函数名可能看起来很奇怪，但这两个函数名来源于最古老的函数式编程语言：Lisp。#strike[这也是 Lisp 的函数命名经常被吐槽的原因之一]
...
我们可以用前文所说的方法将函数转化为一个非递归函数，并用 Y 组合子求值。
...
#strike[如果你试着手打一下这一层套一层的括号，就能理解 Lisp 的痛了]
...
以上文中的`r=(1, 1, 1, 1, ...)`为例。把递归转化成 Y 组合子的形式，就可以得到：
...
如果你使用过 haskell 或者其他支持数组懒惰求值的编程语言，应该或多或少了解过这种写法。它允许你使用有限的语言来描述一个无限长的数列！

_当然，懒惰求值只能表示出*可计算*的无限序列，例如上面说的斐波那契数列和质数数列，或者π的第n位等。这是因为，λ演算的计算能力和图灵机是等价的，任何不能用图灵机算出的数值同样也不能用λ演算算出来。_