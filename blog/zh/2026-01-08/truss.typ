// title: 数值模拟之：桁架结构受力分析
// summary: 桁架的受力分析，就是解一个矩阵方程。
// tags: computation, mathematics, physics, simulation
// category: tech
...
#image("member-force-diagram.svg", width: 70%)

假设每根杆的重力已知，那么这个杆上就有四个未知数：$f_(s x)$、$f_(s y)$、$f_(e x)$ 和 $f_(e y)$，其中矢量 $mathbf(f)_s = (f_(s x), f_(s y))$ 是杆件第一个端点受到的力、$mathbf(f)_e = (f_(e x), f_(e y))$ 是第二个端点受到的力。同时，可以列出杆件的三个受力平衡方程：
...
+ （力矩平衡）
  ...
  $ (f_(s x) - f_(e x)) sin theta - (f_(s y) - f_(e y)) cos theta = 0 $

显然，三个未知数没法约束四个变量，因此我们还需要更多的约束方程。对于每个不固定的铰接节点，其各个方向受的合力均为0，也就是以下两个方程：

+ （x方向合力为0）

  $ sum_("力" i "作用于节点上") f_(i x) = 0 $

+ （y方向合力为0）

  $ sum_("力" i "作用于节点上") f_(i y) = 0 $

举个例子，下面这个简单的结构：

#image("truss-2.png", width: 70%)

设两根杆各自两端共四个力分别为 $f_1$、$f_2$、$f_3$ 和 $f_4$，杆的质量为 $m$，与水平方向的夹角为 $theta$，那么就有：
...
+ （杆1受力平衡）
  ...
  $ (f_(1 x) - f_(2 x)) sin theta - (f_(1 y) - f_(2 y)) cos theta = 0 $
...
+ （杆2受力平衡）
  ...
  $ (f_(4 x) - f_(3 x)) sin theta + (f_(4 y) - f_(3 y)) cos theta = 0 $
...
$ mat(
    ...
    ,,1,,1,,,;
    ,,,1,,1,,
  ) vec(f_(1 x), f_(1 y), f_(2 x), f_(2 y), f_(3 x), f_(3 y), f_(4 x), f_(4 y))
  = vec(0, m g, 0, 0, m g, 0, 0, 0) $
...
#image("truss-4.png", width: 70%)

这个结构同样有12个未知数，但是只有3×3+1×2=11个约束条件。约束条件的个数小于未知数个数，因此这个系统有无限多个解。这里的原因是：我们的模型假定了每根杆都是不会变形的刚体，而理想刚体能够在不变形的情况下承受任意大的力。下面这根直接连接两个固定铰接点的杆上承担的张力实际上会和它的形变程度有关。如果我们在现实中搭建一个这样的系统，即使是 mm 级别的长度差异可能都会在下面的杆上造成 kN 级别的应力，而在理想模型中，即使没有任何形变，这个杆上承载的力都可能是任何数。

= 计算部分：矩阵方程

对于 $M times N$ 的矩阵 $A$ 和 $M$ 维向量 $b$，我们希望寻找一个 $N$ 维向量 $x$，使得 $A f = b$。根据 $M$ 和 $N$ 的相对关系，可以将方程分为下面几类：

+ $M = N$：约束条件数等于方程数。只要矩阵 $A$ 满秩，那么方程就有唯一解。
+ $M < N$：约束条件数小于未知数个数，方程有无穷多组解。
+ $M > N$：约束条件数大于未知数个数，方程大部分情况下无解。
...
令 $(partial f(mathbf(x); mathbf(lambda)))/(partial x_i) = 0$：

#set math.equation(numbering: "(1)")
$ (partial f(x; lambda))/(partial x_i) = 2 x_i + sum_(j=1)^M lambda_j A_(j i) = 0 $ <eq:partial-x>
$ (partial f(mathbf(x); mathbf(lambda)))/(partial lambda_j) = (sum_(i=1)^N A_(j i) x_i) - b_j = 0 $ <eq:partial-lambda>
#set math.equation(numbering: none)

根据 @eq:partial-x 可得：

$ mathbf(x) = -(1)/(2) A^T mathbf(lambda) $

由于 $mathbf(lambda)$ 是我们人为定义的一个向量，它的值可能是任意的。不妨直接令 $mathbf(mu) = -(1)/(2) mathbf(lambda)$，则原式可以写成 $mathbf(x) = A^T mathbf(mu)$。

再带入我们的限制条件，有：

$ A A^T mathbf(mu) = mathbf(b) $

$ mathbf(x) = A^T mathbf(mu) = A^T (A A^T)^(-1) mathbf(b) $

如果 $A$ 是 $M times N$ 的矩阵，那么 $A A^T$ 就是 $M times M$ 的方阵，可以使用常规的矩阵方程求解方法去求解出 $mathbf(mu)$，再对其直接乘 $A^T$ 即可得到 $mathbf(x)$。

== 过约束情况

如果约束条件多于未知数个数，而我们仍然想找到尽可能准确的解，那该怎么办呢？既然没法找到精确解，不妨找一个误差最小的解。

不妨定义 $L(mathbf(x)) = abs(A mathbf(x) - mathbf(b))^2$ 作为我们的目标函数。我们希望找一个使得 $L(mathbf(x))$ 最小的向量 $mathbf(x)$。这是一个无约束的优化问题，并且目标函数的性质足够好，直接寻找梯度为0的点即可。

$ (partial L(mathbf(x)))/(partial mathbf(x)) &= (partial (A mathbf(x) - mathbf(b))^T (A mathbf(x) - mathbf(b)))/(partial mathbf(x)) \
  &= (partial (mathbf(x)^T A^T A mathbf(x) - 2 mathbf(b)^T A mathbf(x) + mathbf(b)^T mathbf(b)))/(partial mathbf(x)) \
...
这个问题也可以看作是求解 $N times N$ 方阵 $A^T A$ 和 $N$ 维向量 $A^T mathbf(b)$ 的矩阵方程，同样可以用标准的方程求解算法来求解。
...
这次我选择了 Rust 作为编程语言，因为它能直接编译成 Webassembly 格式，能在 web 上运行同时不会损失太多性能。线性代数部分使用了 `nalgebra` 库。
...
填充完了矩阵的各个元素之后就可以去求解了。`nalgebra` 库提供了函数 `lu()` 可以直接给矩阵做 LU 分解，而分解之后就可以用 `solve()` 函数逐行代入求解。
...
可视化部分使用了 `web_sys` 包与 canvas 交互。`web_sys` 其实就是用 Rust 写的 Javascript Web DOM 的封装，目前几乎所有可以在 Javascript 中做得操作都可以在 `web_sys` 里找到。

同时我还用一个侧边的 canvas 来可视化上面计算出来的矩阵 $A$ 和 $b$。这里使用了 WebGL 来渲染 canvas，将矩阵 $A$ 和向量 $b$ 并排后作为一个贴图传进去，接着用 WebGL 画一个充满了整个屏幕的四边形，将贴图贴在这个四边形上。这部分的代码过于繁琐，就不展示了。可以在#link("https://github.com/MqCreaple/truss-analysis")[GitHub仓库]里查看源码。
...
一个朴素的思路是，对于方程 $A mathbf(x) = mathbf(b)$，构造一个函数 $g(mathbf(x))$ 使得将 $g(mathbf(x))$ 反复作用于任何向量上之后都可以让它收敛到我们想要的结果上。即构造满足如下条件的序列：

$ cases(mathbf(x)^((0)) in RR^n, mathbf(x)^((n)) = g(mathbf(x)^((n-1)))) $

$ lim_(n -> infinity) mathbf(x)^((n)) = A^(-1) b $
...
$ mathbf(x)^((n)) &= B mathbf(x)^((n-1)) + mathbf(f) \
  ...
  &= dots.c \
  &= B^n mathbf(x)^((0)) + (sum_(i=0)^(n-1) B^i) mathbf(f) $

使用等比数列求和公式，得到：

$ sum_(i=0)^(n-1) B = (I - B)^(-1) (I - B^n) $

$ mathbf(x)^((n)) = B^n mathbf(x)^((0)) + (I - B)^(-1) (I - B^n) mathbf(f) $

不难看出，如果 $lim_(n -> infinity) B^n = mathbf(0)$，那么这个序列就会收敛到：

$ lim_(n -> infinity) mathbf(x)^((n)) = (I - B)^(-1) mathbf(f) $
...
+ $B$ 和 $f$ 应当可以从 $A$ 和 $b$ 两个量中使用简单运算（加减、矩阵乘法、不依赖大量运算的矩阵分解）中得到。
+ $(I - B)^(-1) mathbf(f) = A^(-1) mathbf(b)$
+ $lim_(n -> infinity) B^n = 0$，且 $norm(B)$ 应当尽量小。
...
$ (partial L(mathbf(x)))/(partial mathbf(x)) = A mathbf(x) - mathbf(b) $

不难看出函数梯度仅在 $mathbf(x) = A^(-1) mathbf(b)$ 时为0，即：$L$ 在 $mathbf(x) = A^(-1) mathbf(b)$ 这个点上取最小值。我们任取一个点 $mathbf(x)^((0))$ 作为起点，对其做固定步长的梯度下降，就得到了如下的迭代方程：

$ mathbf(x)^((n)) = mathbf(x)^((n-1)) - lambda (A mathbf(x)^((n-1)) - mathbf(b)) = (I - lambda A) mathbf(x)^((n-1)) + lambda mathbf(b) $

$ cases(B = I - lambda A, mathbf(f) = lambda mathbf(b)) $
...
+ 初始化一个随机的向量 $mathbf(x)$
+ $i$ 从$0$遍历至矩阵宽度 $N$
+ $ x_i <- (1)/(A_(i i)) (b_i - sum_(j < i) A_(i j) x_j - sum_(j > i) A_(i j) x_j) $
+ 重复第2步，直到 $x$ 与目标值足够接近后结束程序
...
$ x_i^((n)) = (1)/(A_(i i)) (b_i - sum_(j < i) A_(i j) x_j^((n)) - sum_(j > i) A_(i j) x_j^((n-1))) $

可能现在看起来还不是很明显。接下来我们将矩阵 $A$ 拆开成一个严格下三角矩阵、一个对角矩阵和一个严格上三角矩阵：

$ A = L + D + U $

$ L_(i j) = cases(A_(i j) & j < i, 0 & "otherwise") $

$ D_(i j) = cases(A_(i j) & j = i, 0 & "otherwise") $

$ U_(i j) = cases(A_(i j) & j > i, 0 & "otherwise") $
...
$ mathbf(x)^((n)) = (D + L)^(-1) (mathbf(b) - U mathbf(x)^((n-1))) $
...
$ mathbf(x)^((n)) = (D + omega L)^(-1) (omega mathbf(b) - [omega U + (omega - 1) D] mathbf(x)^((n-1))) $

$ x_i^((n)) = (1 - omega) x_i^((n-1)) + (omega)/(A_(i i)) (b_i - sum_(j < i) A_(i j) x_j^((n)) - sum_(j > i) A_(i j) x_j^((n-1))) $

通过调整 $omega$ 的值，可以让 $B$ 矩阵尽可能地小，从而让算法收敛得更快。经过实测，使用 SOR 算法可以在不卡顿的情况下计算超过2000个节点。
