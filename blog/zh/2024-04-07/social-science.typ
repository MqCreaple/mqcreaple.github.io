...
// summary: 社会科学里的定量研究，真的更客观吗？
// tags: social-science, mathematics
// category: humanity
...
刚刚结束了今年的 IMMC 比赛（某个数学建模比赛），这次的题目是社会科学相关，使用了很多定量研究方法。由于比赛在现在还没有结束，就不透露任何和比赛题目相关的问题了。我们队这次比赛之前几乎都没有任何社会科学背景，尤其是对定量研究方法很不熟悉，导致比赛中途我们改了很多次模型都没拿到期望的结果。在这里我想简单写一下从这次比赛里我收获的一些知识。
...
我们发现，许多社会科学领域已经有十分成熟的定量研究方法了，比如在经济学中就有例如 GDP 这样的衡量一个经济体的指标。在医学中#strike[虽然医学严格来说不是社会科学]，许多曾经无法定量研究的量也随着科技进步变得可以测量了，比如血常规就可以用来测量一个人身体各项指标是否正常。

即使是在统计学如此发达的今天，仍然有很多问题是我们无法衡量的，尤其是涉及人的主观情绪和观点的学科，如心理学、社会学、政治学。例如，我们（至少目前）没有任何一个能够准确测量一个人性格的指标——不论是 MBTI 还是哪些其他的性格量表都只能描述某个人性格的一个侧面。
...
1. 定类（$=$, $!=$）：如果两个物体A与B并排放置之后发现A和B两端对齐，那么用长度测量A和B就会得到相同的数。
2. 定序（$>$, $<$）：如果将A与B并排放置，一端对齐，发现A的另一端比B更长，那么使用长度来测量A得到的数值就会比B更大。
3. 定距（$-$）：如果物体A与物体B首尾相接之后和物体C一样长，那么物体B的长度就可以表示成物体A和C长度的差。
4. 定比（$div$）：如果 $n$ 个物体A首尾相接后和物体B一样长，那么物体B的长度就是物体A的 $n$ 倍。
...
1. 定类（$=$, $!=$）：如果两个物体A和B处于热平衡，那么A和B的摄氏温标相同。
2. 定序（$>$, $<$）：如果两个物体A和B放在一起，热量从A向B传导，那么A的摄氏温标比B大。
3. 定距（$-$）：如果我们加热一个物体A，不论其起始温度是多少，如果向其传输的热量是一样的，那么这个物体在加热之后的摄氏温标与加热之前的摄氏温标的差距一样。即：把一杯水从20℃加热到50℃的能量和同一杯水从45℃加热到75℃的能量一样大。

但注意，摄氏度没有“定比”属性。你不能说20℃的水就比10℃的水热2倍。虽然此时我们仍然可以把物体温度映射到一个实数上面，但这时两个物体温度的比值就没有意义。

再比如说，测量矿物硬度的时候可能会用到莫氏硬度，它的“定距”和“定比”属性也不能成立——因为莫氏硬度是通过两个物体互相刻划来确定相对硬度的，所以它只能是一个不能“定距”和“定比”的度量。

1. 定类（$=$, $!=$）：如果两个物体A和B相互刻划，两者都有轻微划痕，则两个物体的硬度相同。
2. 定序（$>$, $<$）：如果两个物体A和B相互刻划，A上没有划痕而B上有划痕，那么A的硬度比B大。

莫氏硬度使用了10种不同的矿物来标记硬度的10个级别，但莫氏硬度的数值不能做加减法——它只能用来标记两个矿物硬度的相对关系，而它不保证3级硬度和2级硬度的差与2级硬度和1级硬度的差相同。

== 社会科学的量表

如果说上面三个自然科学中使用的量表都有一个比较直观的定类和定序的方法，那么绝大部分社会科学探讨的问题都是无法定序，甚至无法定类的，比如*智商*。
...
#figure(
  image("Raven-Matrix-sample.svg", width: 70%),
  caption: [莱文方阵（来源：wikipedia）],
)

如果一个人做这样的题比另一个人得分更高，你能说这个人就比另一个人聪明吗？取决于你怎么定义“智商”。“智商”这个概念其实是片面的，人的智力水平是不能简单归结于做智商测试题的能力。

当然也不是所有社会学研究的问题都不能定量，也有许多能够定量描述的社会学参量：经济学里面的收入、存款等等涉及和“财富”相关的量就可以被量化，心理学里一个人脑内激素和神经递质水平也可以相对准确地描述一个人的情绪。这里我主要针对的是使用问卷和量表等“看起来很客观”的测量方式对一个主观概念做定量分析的研究方法，比如上文提到的智商测试，又或者是最近非常流行的 MBTI 人格测试这些。
...
2. 可复制：别的研究者可以使用同样的研究方法研究同样的问题而得到相同的结果

但一个定量社会科学研究真的能做到这两点吗？或许，符合这些要求的研究并不多。抛开在收集数据过程中可能出现的各类误差（如：无反应误差）不谈，光是“怎么定量”就问题重重。

== 提问方式

在很多情况下，对于同一个问题，不同的提问方式可能会带来显著的回答差异，大到用统计学方法能明显地看出区别。比如，在某个研究里面，当问及受试者“你是否幸福”的时候，用-5到5做答案区间的话就会比用0到10做答案区间得到的结果换算到同一个区间之后更高 @cite1。类似的例子还有很多。
...
统计学固然在分析宏观数据分布和趋势时非常有用，但只关注统计数据而忽略掉无法统计的因素也会出问题。维基百科上有一条名为#link("https://en.wikipedia.org/wiki/McNamara_fallacy")[麦克马拉谬误]的条目，又称“量化谬误”，指的是过度相信统计数据而忽略了统计数据之外的其他因素对某个变量的影响。
...
即使上述所有误差我们都尽可能地避免，最终得到的数据也保证尽可能准确，仍然还有从_观测_到_决策_这一步。自然科学里面你的观测数据不会影响到自然本身的运行，但在社会科学里，你的观测数据和你对数据的解读是会切实影响到其他人的决策。一个国家的经济可能真的会因为研究者预测其会增长而真的逐渐增长。此时，为了能让研究结果在付诸实践的时候产生尽可能小的负面影响，准确区分两个变量的_相关性_和_因果性_就显得格外必要。

出于伦理道德的要求和人性的约束，社会科学里很难做大规模实验，而只用对比观察得出的结论很难区分两个变量到底仅仅是_相关_还是有_因果关系_。若两个变量没有因果关系，而是某个（或多个）隐藏变量导致了两者，又或者是巧合，那么实施政策的一方试图改变第一个变量时就不一定会达到预期的效果，反而有可能产生意想不到的影响。关于相关性和因果性的讨论在统计学里面应该已经老生常谈了，我在此也不多赘述。

#quote(block: true)[
其实“统计学”这门学科的来源并不是很干净——最早的统计学方法除了用在统计国家的经济数据以外，也被用来合理化种族歧视、优生学、甚至于种族灭绝。B站up#link("https://space.bilibili.com/35868098")[长河劫]的#link("https://www.bilibili.com/video/BV1KS421P7sN/")[这个视频]就讲了这一段历史。我认为，每一个从事社会研究的人都应该从这段历史中学到一件事：*科学方法不能让人失去人性*。这一点在研究人类自身的社会科学里尤其重要。
]
...
另一方面，机器学习方法也无法完全避免上述的问题，像是过度依赖定量和统计。机器学习方法也有一些其独有的局限性：由于机器学习完全依赖喂给它的数据，如果这些数据里面有系统性的偏差，那么模型训练出来也会包含这些偏差。像是我最近在看的一本书《人机对齐》（_The Alignment Problem_）里面所说的，由于人类社会中对特定职业的性别偏见（比如通常认为程序员是男性，护士是女性），用大量人类语料训练出来的 word2vec 程序里面就会把特定职业和特定性别联系起来，而如果此时某个相关职业的机构（如：互联网公司）使用 word2vec 算法来筛选求职者的简历，导致筛选出来的简历里男性多于女性，就会进一步加剧性别刻板印象。
