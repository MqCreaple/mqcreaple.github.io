...
3. 使用某种凸优化方法（例如梯度下降法）更新$q_i$，使得新的路径$q_i$上的$S[q]$相较于原来更小。比如用朴素梯度下降算法：

$$q_i^{(t+1)}=q_i^{(t)}-\eta\frac{\partial S[q_i^{(t)}]}{\partial q}$$

**重要声明：上述的所有方法和数学推导均为100%原创，但我不确定是否有前人发明过同样的或者类似的方法。如果有任何读者了解过其他类似的使用泛函凸优化的数值模拟方法，也欢迎在主页评论区留言。**
